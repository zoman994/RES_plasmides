 ...
 * to re-ligate). Linear fragments and fragments inside multi-fragment assemblies
 * must go through two_fragment / multi_fragment overlap PCR.
 *
 * @param {Array}  mutations
 * @param {Object} [fragmentContext]
 * @param {'circular'|'linear'} [fragmentContext.topology='circular']
 * @param {boolean}             [fragmentContext.isStandalone=true]
 * @param {number}              [fragmentContext.length]
...
export function computeMutagenesisStrategy(templateSeq, mutations, options = {}) {
  ...
  return makeFragmentStrategy(templateSeq, sorted, featureStart, featureEnd, overlapLength, bindingLength, strategy);
}
...
function makeFragmentStrategy(templateSeq, mutations, featureStart, featureEnd, overlapLength, bindingLength, strategyLabel) {
  const warnings = [];
  ...
  // Cut points — midway between adjacent mutations. For single mutation (linear
  // or non-standalone case), cut AT the mutation position so it ends up in the
  // overlap zone between the two resulting fragments.
  const cutPoints = [];
  if (mutations.length === 1) {
    cutPoints.push(mutations[0].dnaPosition);
  } else {
  ...
  return {
    strategy: strategyLabel || (mutations.length === 2 ? 'two_fragment' : 'multi_fragment'),
    fragments,
    junctions,
