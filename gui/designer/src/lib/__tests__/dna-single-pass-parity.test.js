/**
 * U7 — the SINGLE_PASS route must be indistinguishable from the shipped EXACT_FIRST route.
 *
...
function bothRoutes(q, doc, ctx) {
  const out = {};
  for (const [key, route] of [['exactFirst', SEQUENCE_ROUTE.EXACT_FIRST], ['singlePass', SEQUENCE_ROUTE.SINGLE_PASS]]) {
...
const agree = (q, doc, ctx, label) => {
  const { exactFirst, singlePass } = bothRoutes(q, doc, ctx);
  expect(singlePass, label).toEqual(exactFirst);
  return exactFirst;
};
...
describe('U7 parity — the four outcomes the route can produce', () => {
...
describe('U7 parity — the shapes that could break the superset claim', () => {
  ...
  it('BLOCKER — on a repeat-rich molecule single-pass turns a cheap exact answer into RESOURCE_LIMIT', () => {
    // THE REASON SINGLE_PASS IS NOT ADOPTABLE AS WRITTEN, pinned as a fact rather than prose.
    //
    // The superset claim is true — the approximate scan does find every exact candidate — but it
    // says nothing about COST. Exact-first answers this query with a k=0 band (one diagonal, no
    // frontier); single-pass is forced through the full approximate DP first, and on a tandem
    // array every repeat copy spawns its own ladder of shifted starts. The verifier axis is
    // exhausted before the answer is reached, so a search that today returns 229 exact loci comes
    // back "incomplete" instead. For a biologist searching a repeat unit in a repetitive plasmid
    // that is a straight regression.
    //
    // This assertion is deliberately written as the DIVERGENCE. If a future implementation makes
    // single-pass affordable here, this test starts failing and forces the blocker to be retired
    // explicitly instead of quietly.
    const unit = 'ACGTTGCAACGTTGCAACGTTGCA';
    const tandem = unit.repeat(40);
    const { exactFirst, singlePass } = bothRoutes(unit + unit, doc(tandem), CTX);
    expect(exactFirst.code, 'exact-first answers this cheaply today').toBeNull();
    expect(exactFirst.hits.length).toBeGreaterThan(0);
    expect(exactFirst.hits.every((h) => h.identity === 1)).toBe(true);
    // CONTRACT CHANGE (U6-F.1). This used to be `RESOURCE_LIMIT`: single-pass ran the approximate DP
    // on a repeat-rich molecule and the PRODUCTION verifier could not afford it, which is why the
    // route was documented as unshippable. The linear verifier — now the default kernel — computes
    // one shared window instead of one DP per near-identical start, so the refusal no longer
    // reproduces. The case is kept, and inverted, because it records WHY the route was blocked and
    // what changed; deleting it would erase both.
    expect(singlePass.code, 'the linear verifier can afford what the production one could not').toBeNull();
    expect(singlePass.hits.length).toBeGreaterThan(0);
  });
...
describe('U7 parity — routing statuses must not blur', () => {
  it('REQUIRES_ALIGNMENT for a >100 nt approximate query is preserved', () => {
    const q = mutate(BG.slice(500, 800), 100); // 300 nt, no exact hit
    const { exactFirst, singlePass } = bothRoutes(q, doc(BG), CTX);
    expect(exactFirst.code).toBe(REQUIRES_ALIGNMENT);
    expect(singlePass.code, 'the length route must survive the single-pass seam').toBe(REQUIRES_ALIGNMENT);
  });
  ...
  it('INVALID_DNA is still INVALID_DNA, on both routes', () => {
    const { exactFirst, singlePass } = bothRoutes('ACGTNACGTACGT', doc(BG), CTX);
    expect(exactFirst.code).toBe('INVALID_DNA');
    expect(singlePass.code).toBe('INVALID_DNA');
  });
