...
export default function PartsPalette() {
  ...
  const nonPlasmidParts = filtered.filter(p => !isPlasmid(p));
  const grouped = groupByCategory(nonPlasmidParts);
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {CATEGORY_ORDER.filter(cat => grouped[cat]?.length > 0).filter(cat => expertMode || STUDENT_CATEGORIES.has(cat)).map(cat => (
          <div key={cat}>
            ...
            {grouped[cat].filter(p => !p.parentId && (!p.parentIds || p.parentIds.length === 0)).map(p => {
              ...
              return (
              <div key={p.id}>
                {/* Part row — click to expand + highlight, drag to add */}
                ...
                {/* Expanded: derivation tree + info + actions */}
                {isExpanded && (
                  <div className="mb-2">
                    {/* ── Children tree ── */}
                    {children.length > 0 && (
