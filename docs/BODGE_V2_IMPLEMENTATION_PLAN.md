...
Legacy `deviceId`, blob URLs, inline expected/actual Sanger sequences и generic `verified` checkbox не переносятся. Structured Sanger/sequence result становится evidence E4, observed sequence — container, а notebook лишь ссылается на них. Registry metadata живёт в том же notebook asset; bytes, MIME/magic и omission projection реализуются F3. Old `SPEC_BODGE_NOTEBOOK_MARKDOWN.md` используется только как UX/rendering reference, не как data contract.
...
0. **Граница задачи** — дождаться приёмки/фиксации активного Search sprint и создать отдельный `CURRENT_TASK`; Search и Format не смешиваются в одном diff.
...
Перед каждым отдельным K-шагом восстановить обязательный project context по `AGENTS.md`, затем прочитать этот plan, нормативную спецификацию и только фазово-релевантные modules/tests. `CURRENT_TASK.md` должен содержать один ограниченный slice, а не всю программу интеграции.

Карта чтения:

- **G0/A, ZIP/security/conformance:** `gui/designer/src/lib/bodge-zip.js`, `bodge-manifest-v2.js`, `bodge-recovery.js`, `bodge-atomic-write.js`, `bodge-export-profiles.js`, `file-system.js`, schema и существующие `bodge-*.test.js`; на Python-стороне — `pyproject.toml` и новый изолированный `src/pvcs/bodge_cli/` без импорта browser validator code.
- **B, canonical project/container:** `CanvasSkeleton/lib/skeleton-bodge-bridge.js`, `store/skeleton-state*.js`, `store/projectSlice.js`, `store/projectAssembliesSlice.js`, `store/primerSlice.js`, `annotation-model.js`, `segment-annotation-transfer.js`, `bodge-hash.js`, `file-import.js`, `genbank-parser.js`, `sequence-utils.js`, `lib/iupac.js` и coordinate/annotation tests. Последние helpers читаются как compatibility traps: silent strip/U→T/default-linear не переносятся в core.
- **C, GenBank/import boundary:** `bodge-container-genbank.js`, `export-genbank.js`, `genbank-parser.js`, backend `src/pvcs/snapgene_parser.py`, `src/pvcs/parser.py`, `src/pvcs/models.py`, `src/pvcs/config.py`, `gui/api/server.py` и реальные interop fixtures/loss matrix. Любой default topology/strandedness в этих границах проверяется отдельно.
- **D, assembly/chemistry:** `bodge-assembly-json.js`, `bodge-assembly-portable.js`, `CanvasSkeleton/lib/assembly-model.js`, `piece-model.js`, `assembly-utils.js`, `local-primer-design.js`, `golden-gate.js`, `restriction-db.js`, `mutagenesis.js`, `validate.js`, operation/junction state, selectors/actions и chemistry validators. Portable module изучается как legacy behavior, но не расширяется как второй reader; Type IIS/RE separation, KLD circular-backbone gate и запрет merge через ligation входят в обязательные D tests.
- **E, provenance/evidence/notebook:** Sanger helpers/actions/selectors и `components/CanvasSkeleton/canvas/SangerLabNotebook.jsx`; container history/version paths; `store/primerSlice.js`; `canvas/NotebookEntryEditor.jsx`, `NotebookTab.jsx`, `NotebookRefPickerModal.jsx`; `lib/notebook-migrations/t10-to-notebook.js` только как characterization input, не production converter; attachment/source modules, `bodge-zip-notebook-attachments.test.js`, notebook UI/dependency tests и cross-language digest golden vectors. Legacy `text/author.deviceId/data.sangerVerified` shape не переносится как canonical contract.
- **F, persistence/cutover:** `App.jsx`, `components/AppShell/Topbar.jsx`, `StartScreen/StartScreen.jsx`, `StartScreen/Sidebar.jsx`, `StartScreen/MainPanel.jsx`, `StartScreen/ProjectContextBar.jsx`, `StartScreen/HelpPopover.jsx`, `StartScreen/lib/open-bodge.js`, `Library/LibraryWorkspace.jsx`, `components/ProjectInfoModal.jsx`, `Library/tree/LibraryTreeRoot.jsx`, `Library/tree/ProjectZone.jsx`, `Library/tree/TrashZone.jsx`, `Library/hooks/useSearchPickRouter.js`, `canvas/ExportProjectModal.jsx`, `components/CanvasSkeleton/index.jsx`, `SettingsModal.jsx`, `MultiTabBlocked.jsx`, `ReadOnlyForced.jsx`, `lib/bodge-assembly-portable.js`, `lib/search-pick-route.js`, `lib/open-entry-action.js`, `lib/hotkeys.js`, `store/index.js`, `db/dexie-schema.js`, `skeleton-context.jsx`, `skeleton-persistence.js`, `multi-tab-lock.js`, `projectSlice.js`, `projectAssembliesSlice.js`, `librarySlice.js`, `primerSlice.js`, lifecycle/multi-tab/activation tests, File System Access/OPFS adapters и browser lifecycle tests. Settings explicit user reset и format cutover reset — разные commands. До кода freeze-ятся request discriminants, diagnostics/actions, progress/cancel, dirty/binding states и commit fences; Search UX не меняется, меняется только typed project activation boundary.

Перед изменением annotation/assembly/chemistry code обязательны project contracts `annotation-contract`, `bio-invariants` и `assembly-methods-ref`; для любого production code — `tdd-enforce`, `size-budget` и `scope-stop`. Существующие `bodge-migrations/*` и migration tests являются объектом characterization/removal, а не основой нового reader.

Каждый handoff заканчивается targeted tests фазы, полным `npm test`, production build, size report и проверкой, что не появился новый live bypass. До F7 допустимы только dark modules/tests; изменение App/open/save callsites раньше cutover является scope violation.
