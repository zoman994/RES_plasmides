...
**Статус:** ГОТОВО К ПОЭТАПНОМУ HANDOFF после закрытия текущего active sprint

**Тип:** Type A — архитектура, data model, export pipeline, UX workflow

**Дата ревизии:** 13.07.2026

**Заменяет:** `SPEC_REPRODUCIBLE_RECIPE.md` v1 от 17.06.2026
...
Это master-spec/epic, а не один sprint handoff. Её намеренно нельзя отдавать Code командой «сделай всё»: `CURRENT_TASK.md` получает только один bounded gate из `BODGE_V2_IMPLEMENTATION_PLAN.md`, после реализации — отдельная приёмка и следующий срез. Тем самым локальная реализационная спека остаётся в лимите `SPEC_CHECKLIST.md`, а полный продуктовый замысел не распадается на противоречащие документы.

### 0.1 Размеры затрагиваемых модулей на 13.07.2026

| Модуль | Размер | Решение для этой работы |
|---|---:|---|
| `lib/bodge-zip.js` | 25 639 B | На границе hard 25 KB. Новую логику не добавлять внутрь; оставить dispatcher, publication writer вынести. |
| `lib/bodge-export-profiles.js` | 9 455 B | Переписать профиль `public-supp` на allowlist + dependency closure. |
| `lib/bodge-assembly-json.js` | 9 006 B | Использовать как текущий adapter, затем привести к нормативному reaction graph. |
| `lib/bodge-container-genbank.js` | 13 303 B | Переиспользовать для expected и observed containers. |
| `lib/bodge-readme-writer.js` | 7 435 B | Расширить через отдельные publication renderers, не превращать в mega-template. |
| `canvas/ExportProjectModal.jsx` | 9 798 B | Переиспользовать как точку входа; guided publication flow вынести в дочерние шаги. |
| `store/alignmentSlice.js` | 20 937 B | Уже в soft-zone. Не хранить publication/evidence state здесь. |
| `components/Align/AlignWorkspace.jsx` | 7 720 B | Переиспользовать импорт/визуальный язык, но не делать его владельцем проверки плазмиды. |
| `components/Align/AlignResultView.jsx` | 20 090 B | Не наращивать whole-plasmid workflow внутри компонента. |
| `lib/alignment/align-circular.js` | 2 688 B | Полезен для placement, но не является полноразмерным global comparator. |
| `CanvasSkeleton/lib/skeleton-bodge-bridge.js` | 4 736 B | Расширить переносом canonical evidence/publication refs; убрать зависимость publication от lossless extension. |
| `ProjectAssemblyWorkspace.jsx` | 5 436 B | Одна из точек вызова «Паспорт конструкции». |
| `SangerLabNotebook.jsx` | 5 028 B | Показывать scoped evidence; не считать ручной `verified` whole-molecule доказательством. |

**Size decision:** публикационная фича не должна увеличить `bodge-zip.js` и `alignmentSlice.js`. Если изменение любого из них даёт положительный рост, STOP: orchestration/DTO/filtering выносится в новый pure module.
...
- legacy PlasmidWorkspace/PlasmidUseWizard для новой поверхности;
...
Первая реализационная задача после текущего canvas sprint — не весь epic и не UI паспорта, а **G0 characterization + A1.0/A1.1 schema inventory/registry core** из implementation plan. Следующий bounded sprint выбирается только после gate предыдущего. Whole-plasmid comparison и production publication export не начинать, пока canonical graph/profile contract не зелёный.

Code обязан прочитать:

- `CHAT_PLAYBOOK.md`;
- `AGENTS.md` / `CLAUDE.md`;
- `BUGS.md`;
- актуальный `CURRENT_TASK.md`;
- `docs/SPEC_BODGE_FORMAT_V2_CORE.md`;
- этот документ;
- `docs/BODGE_V2_IMPLEMENTATION_PLAN.md`;
- `docs/COMPONENT_MAP.md`;
- skills `bio-invariants`, `assembly-methods-ref`, `tdd-enforce`, `size-budget`, а для UI также `design-system` и `ui-interactions`.

**Будущая handoff-фраза после финализации текущего спринта:**

> Выполни только G0 characterization и A1.0/A1.1 schema inventory + registry core по `BODGE_V2_IMPLEMENTATION_PLAN.md`, TDD red→green. Зафиксируй текущие profile/writer/skeleton/Sanger/comparator gaps; создай один strict local registry, manifest/recovery compile gate и stable error normalization; production reader/writer/UI пока не переключай. Canonical graph остаётся единственным целевым источником истины; `recipe.json` не создавать; `realiseAssembly` не использовать как replay validator. Biological schemas A1.2/A1.3, Construction Passport UI, publication ZIP, whole-plasmid comparison, raw reads, LIMS и autolab — OUT этого среза. После gate A1.1 остановись; не финализируй PROJECT_STATE/RELEASES/DECISIONS/ANCHORS/BUGS и не начинай A1.2/A2/P1. Тело спецификаций не переписывай.

Формат отчёта — `docs/process/CODE_HANDOFF_PROTOCOL.md` §4; после последнего K-шагa применяется `sprint-report` и `scope-stop`.
