...
**Статус:** 🟡 PROGRAM SPEC COMPLETE; HANDOFF BLOCKED  
**Тип:** Type A; фазы — спринты  
**База аудита:** `v0.8.4-alpha`, code/filesystem snapshot начат 16.07.2026, `HEAD 0085bec`; worktree занят Smart Search  
...
> **Handoff blocked.** M-ALIGN не входит в `CURRENT_TASK.md`, пока активна иная задача; сейчас это `REV #2 SEARCH-COMMIT-AUDIT / K1`. До handoff: новые `BUGS.md` IDs, включая `align-routing.test.jsx`; docs rotation `28→<=8`; D0 repository/schema/quality gates. Search-файлы и трекеры не меняются.

> **Масштаб.** Фазы A–E получают отдельные handoff 5–10 KB, STOP и приёмку. Тело спеки Code не редактирует; ledger ведёт Chat.

| Фаза | Состояние до handoff | Причина |
|---|---|---|
| A — hardening/worker | blocked | активная non-Align задача, docs rotation, BUGS IDs |
| B — Sanger QC/lifecycle | blocked by A | production verdict дополнительно ждёт quality policy |
| C — multi-read | blocked by B | corpus acceptance |
| D — persistence/evidence | blocked by C + D0 | canonical repository и Bodge `2.0.0` schema delta |
| E — conformance | blocked by D | полный продуктовый поток ещё не принят |

---

## 0. Срез размеров и границы файлов

Размеры сняты с filesystem snapshot до реализации.

| Файл | Bytes | Лимит / решение |
|---|---:|---|
| `components/Align/AlignInputPanel.jsx` | 18 265 | OK |
| `components/Align/AlignMiniMap.jsx` | 5 171 | OK |
| `components/Align/AlignReferenceView.jsx` | 12 007 | OK |
| `components/Align/AlignResultView.jsx` | 20 090 | OK; новые панели отдельно |
| `components/Align/AlignWorkspace.jsx` | 7 720 | OK |
| `components/Align/SaveCorrectedVersionModal.jsx` | 6 296 | OK |
| `store/alignmentSlice.js` | 20 937 | JS soft; декомпозиция первым шагом A1 |
| `lib/alignment/align-pairwise.js` | 17 096 | OK |
| `lib/alignment/{anchor,wfa,align-circular,align-to-reference}.js` | 4 632 / 8 204 / 2 688 / 1 765 | OK |
| `lib/alignment/{multi-align,poa,abif-parse,double-peaks,alignment-quality}.js` | 6 368 / 7 609 / 4 282 / 2 309 / 3 573 | OK |
| `components/SequenceView/index.jsx` | 72 845 | Active hard breach; **OUT, не редактировать** |
| `store/librarySlice.js` | 54 398 | Watch hard breach; **OUT, не расширять** |
| `lib/{search-worker-client,annotator-worker-client}.js` | 10 408 / 3 669 | A2: только scheduler adapters |
| `db/dexie-schema.js` | 14 818 | только D после repository gate |
| `components/CanvasSkeleton/canvas/SangerLabNotebook.jsx` | 5 028 | только D3 |
| `lib/bio/sanger-primer-design.js` | 6 439 | переиспользовать |

Правила:

1. A1 выносит contracts/fingerprint/session orchestration из `alignmentSlice.js`; новая алгоритмическая логика в slice запрещена.
2. Новый core-prop в `SequenceView/index.jsx` требует отдельного decomposition sprint и STOP. Quality/coverage живут в Align adapters/leaf tracks существующего API.
3. `librarySlice.js` не меняется. Если существующего write API недостаточно, работа останавливается до отдельного size/decomposition решения.
4. Новые `.jsx` и `.js` соблюдают 40/30 KB и 25/20 KB hard/soft budgets.
...
UI-реализация использует skills `design-system` и `ui-interactions`; annotation edits — `annotation-contract`; любой код — `tdd-enforce`, финализация — `size-budget` и `scope-stop`.
...
Существующие canonical записи: [`BUGS.md`](../BUGS.md) `AUD-12`, `AUD-23`, `AUD-90`, `AUD-91`, `AUD-92`. Перед A handoff в `BUGS.md` отдельно регистрируются остальные подтверждённые дефекты; эта спека не является вторым bug tracker.
...
Последний focused audit дал `222 passed / 1 failed`: `align-routing.test.jsx` дважды завершился timeout 5000 ms, включая isolated run. Это не доказывает ошибку alignment math, но закрытый V146 нельзя считать детерминированным. A1 исправляет ожидание chunk/route через controlled preload/mock/await; простое увеличение timeout запрещено.
...
- `SangerLabNotebook.jsx` и `sanger-primer-design.js` только D3.
...
Для каждого изменения: RED focused test → minimal GREEN → полная проверка:

```bash
cd gui/designer && npm test && npx vite build
```

Один файл: `npm test -- <path>`, не `npx vitest`. После code changes: `npx graphify hook-rebuild`. Python не меняется; report пишет `pytest: not touched`, иначе точный N/N. Browser acceptance обязательна для worker/UX/performance.

Code MAY менять в `BUGS.md` только IDs текущей M-ALIGN-фазы и отмечать `[x]` лишь реально исправленные; соседние bugs/документы не трогаются. После каждого phase STOP:

```text
## Отчёт Code — M-ALIGN-TRUST Phase <A|B|C|D|E>
- commits и scoped files
- RED proof; focused tests
- full Vitest; vite build; pytest
- browser scenarios и console
- differential/property/corpus result
- declared-machine performance, memory, long tasks, cancel p95
- files before→after; new files/sizes; size budget
- graphify hook-rebuild result
- BUGS IDs fixed/open
- deviations/unresolved diagnostics
- expected phase-ledger/status update by Chat
...
1. **Mega-sprint/docs drift.** Отдельные phase handoff/STOP; docs rotation до реализации.
...
- [`DECISIONS.md`](../DECISIONS.md) — DEC-ALIGN-01…08, DEC-ALIGN-PERF-01…08;
...
- [`ARCHITECTURE.md`](./ARCHITECTURE.md) — local-first и performance manifest;
- [`DESIGN_SYSTEM.md`](./DESIGN_SYSTEM.md) — UI tokens;
- [`TECH_DEBT.md`](../TECH_DEBT.md) — size/decomposition status.
