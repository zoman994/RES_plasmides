...
**Статус (на 26.04.2026):** ✅ ЧАСТИЧНО РЕАЛИЗОВАНО. Phase 1 (Блоки 8–11) ✅ done. **Phase 2** (PCRNode + AssemblyNode + CheckpointNode + 3 edge types + MIRO+) ✅ — соответствие подтверждено по `PROJECT_STATE.md` блок «Project Flow»: 5 node types (PlasmidNode/PCRNode/AssemblyNode/OligoNode/CheckpointNode), 3 edge types, dagre layout, PCR node edit, real assemblies, MIRO+ с RE/KLD/лигированием. **Phase 3** (планирование с статусами pending/in_progress/done/failed/skipped + симуляция timing + critical path) — не реализовано на момент архивации, отложено на v1.1+. Эта спека сохранена в archive для возврата к Phase 3 при необходимости.

**Спека исходно от 31.03.2026.** Архивировано 26.04.2026 в рамках финализации цикла Sprint X cycle (docs/ cleanup).

---

# Project Flow Canvas — Фазы 2+3: Операции + Режим планирования  
**Дата спеки:** 31 марта 2026  
**Зависимости:** Block 11 Phase 1 (PlasmidNode, ProjectFlowCanvas, projectFlowSlice) — выполнено  
...
## Reference: идеи из архивированной FLOW_V2_DESIGN.md

Ранее существовала альтернативная архитектурная концепция `FLOW_V2_DESIGN.md` (спека 31.03.2026, архивирована 22.04.2026) — один универсальный `ReactionNode` с configurable operation вместо отдельных типов нод. Концепция отвергнута в пользу 5-node-схемы (PlasmidNode/PCRNode/AssemblyNode/OligoNode/CheckpointNode), которая работает. Возврат к Universal не планируется. Ниже — три идеи из той спеки, которые переживают смену архитектуры и полезны как reference для будущих фаз.

**1. Инвентарь будущих типов нод (14 операций).** FLOW_V2 перечислял 14 lab-операций: source, pcr, digest, ligation, gibson, golden_gate, kld, dpnI, gel, purification, transformation, colony_pcr, sequencing, miniprep. Сейчас реализовано боттом 4–5: source (PlasmidNode), pcr, assembly-как-одна-нода (gibson/gg/kld внутри), primer (OligoNode), checkpoint. **Кандидаты на новые ноды** (по приоритету для lab workflow): `DigestNode` (отдельно от assembly, когда restriction — это цель, не часть сборки), `TransformationNode`, `ColonyPCRNode`, `SequencingNode`, `DpnINode` (специфично для mutagenesis cleanup). `GelNode`/`PurificationNode`/`MiniprepNode` — возможно избыточно, вдуматься.

**2. Primer auto-design при подключении PCR-ноды к template.** FLOW_V2 предлагал флоу: пользователь тянет edge от PlasmidNode к PCRNode → автоматически lookup в primer registry → если нашёлся праймер на эту темплейт — показать «✅ IS001 fwd (в наличии)»; если нет — предложить «🆕 подобрать новые» через `findBinding()` + auto-design. В Construct view примерный флоу есть в ассембли, но в Flow view отдельного сценария нет. **UX-кандидат** для Phase 2/3: при создании edge `plasmid → pcr` автоматически запрашивать праймеры, отображать их на primer-handles ноды сверху.

**3. Status tracking нод (planned → in_progress → done → failed).** FLOW_V2 закладывал явный lifecycle-статус на каждой ноде: click to cycle planned→in_progress→done→failed. Сейчас status есть только на CheckpointNode, на PCR/Assembly/Oligo — нет. Для lab workflow это важно: пользователь должен видеть на канвасе что уже сделано, что идёт, что провалилось. Пересекается с PARTS_LIFECYCLE.md (там аналогичный lifecycle для запчастей), но на уровне нод Flow — отдельная фича. **Кандидат** для Phase 3.

_Самому FLOW_V2_DESIGN.md в archive/ не доверять как плану реализации — он всё ещё описывает отвергнутую Universal ReactionNode-архитектуру._
