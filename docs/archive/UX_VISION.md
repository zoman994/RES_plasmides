...
## 0. Философские якоря — беречь от забывания

Записи без roadmap привязки. Это не «что делать в M-X.N» — это «зачем вообще делаем BodgeGene». Подводные камни когда через год биолог/Chat начнёт забывать.

### 0.1 Две метафоры DAG — расходящаяся и сходящаяся

Биолог держит в голове две одновременные ментальные модели проекта:

1. **Расходящаяся (DAG-as-tree, git branches).** «Из этой плазмиды я делаю варианты». В академическом protein engineering это основной вид — SnapGene + Benchling рисуют только её. Реализуется через `Dag/` (M-C.1) сейчас.

2. **Сходящаяся (DAG-as-recipe, виртуальный штатив).** «Беру пробирку A + пробирку B + master mix → гет ligation product C». Биолог это делает руками в лабе каждый день. Игорь со своими студентами заставляет рисовать это от руки для проверки понимания — если студент не может нарисовать pipeline, он не понимает что делает. Никто из конкурентов эту метафору не визуализирует — SnapGene/Benchling изначально строились как замена форматированию GenBank, не labbook'у.

**Следствие для UX:** Mix workspace (M-E) и DAG polish (M-I) должны поддерживать обе ментальные модели без компромиссов. Биолог иногда смотрит на проект как на дерево («в каком месте у меня появилась эта мутация»), иногда как на штатив («что мне сегодня в какую пробирку наливать»). Одна метафора без другой — неполный инструмент.

**Реализация второй метафоры** (виртуальный рабочий стол / штатив) — **post-v1.0 фича**, не раньше. Причины: (a) нужны accumulated DAG-data реальных проектов (десятки узлов, разные топологии) для honest дизайна; (b) M-C.2 + M-E фундамент (Container Window и Mix workspace) должен быть рабочим — без них в «пробирки» нечего класть; (c) UX-паттерн нестандартный, требует exploration и прототипов; (d) risk over-design при раннем решении на hypothetical use cases.

**Не забывать:** когда биолог в какой-то из v0.9..v1.0 спринтов скажет «неудобно в DAG понять что сейчас в холодильнике / в термоблоке / готово» — это сигнал что вторая метафора набрала критическую массу и пора в роадмап.

**Добавлено:** 10.05.2026 по рефлексии Игоря о том, как он преподаёт студентам через рисование pipeline от руки.
