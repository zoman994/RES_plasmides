...
- [`SPEC_BODGE_NOTEBOOK_MARKDOWN.md`](./SPEC_BODGE_NOTEBOOK_MARKDOWN.md);
- [`BODGE_V2_PROVENANCE_SCHEMA_PLAN.md`](./BODGE_V2_PROVENANCE_SCHEMA_PLAN.md);
...
Markdown хранится как inert text. Conforming renderer MUST отключать raw HTML execution, remote image/resource auto-fetch и scriptable schemes; `https` link открывается только как ordinary external navigation. Дополнительный [`SPEC_BODGE_NOTEBOOK_MARKDOWN.md`](./SPEC_BODGE_NOTEBOOK_MARKDOWN.md) описывает UI/renderer идеи, но его legacy field names и historical process sections не изменяют этот нормативный shape.
