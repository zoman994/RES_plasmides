...
## Dead code: DAG workspace orphan (17.06.2026)

- **TD-DEAD-DAGWORKSPACE** (DONE — компоненты+enum, 17.06.2026): legacy DAG-воркспейс снесён. Шаг 1 (входы): убраны кнопка «→ в DAG» (`ProjectZone`), действие «Показать в DAG» (`library-actions`), маршрут `'flow'` (`AppShell`) и overlay `case 'dag'` (`App.jsx`). Шаг 2 (sweep): удалены 7 DAG-only компонентов (`DagWorkspace`, `DagCanvas`, `NeutralEdge`, `PlasmidNode`, `PreviewDrawer`, `Palette/DagPalette`, `Palette/DagPaletteItemRow`) + 6 тестов (5 component-тестов + `strings-and-edges` — edge-deletion дублировался в `projectSlice.dag.test`); `'flow'` убран из `VALID_WORKSPACES` (`workspaceSlice` + `workspaceSlice.test` перешит на `'align'`/`'construct'`); `'dag'` убран из `FULLSCREENS` + мёртвое поле `canvas.dagViewport` (`canvasSlice`); `containerWindow-route.test` перешит с `'dag'` на `'library'`. **Сохранены:** `ContainerWindowPlaceholder` (+2 теста, ещё используется `App.jsx`); `lib/dag-layout.js` (**shared** с CanvasSkeleton `canvas-layout`/`zone-layout` — НЕ удалять); store DAG-модель (ниже). `DEC-MX7A-V2-09` (DAG sub-row wiring) — отозван. Прогон 4895 pass / 0 нов. fail (только pre-existing flake V146); build clean (precache 36→32).
  - **Остаток (TD-DEAD-DAG-STORE-MODEL, OPEN, low):** Project.dag-модель (`project.dag.{nodes,edges,viewport}` + действия `setDagPositions`/`addDagEdge`/`removeDagEdge`/`setDagViewport`/`addContainerToCurrentProject`) теперь UI-сирота — гоняется только тестами (`projectSlice.dag.test`). Снос требует трогать persistence/.bodge round-trip + миграцию → отдельный заход. Также `STRINGS.dag` частью мёртв: живы только `containerWindow*` (их ест `ContainerWindowPlaceholder`), остальные ключи (emptyHint/palette*/drawer*/autoLayout/toastAlreadyOnCanvas/drawerRegionOverflow) — unused data; обрезать вместе со store-моделью.
  - **Окно:** при заходе на persistence/format-чистку. Не блокер.
...
**Snapshot 17.06.2026 (PlasmidMiniMap geometry decomp — supersedes 02.06):**
- **Reopened → Closed decomp'ом:** **TD-SIZE-PLASMID-MINI-MAP** был закрыт 08.05.2026 на 29.68 KB с пометкой «если M-C/M-D вернут рост — реоткрыть». SnapGene-overview работа 17.06 (bp ruler + strand-arrows вариант A + center-label + click-to-set-origin) разогнала `components/PlasmidMiniMap.jsx` 29.1 → **46.7 KB** (hard breach 40, rate-of-change триггер) → **DONE**: вся чистая геометрия вынесена байт-в-байт в новый `lib/plasmid-mini-map-geometry.js` (**10.5 KB**, под soft 20). Компонент → **39.02 KB** (под hard 40, ~1 KB запас; остаётся Watch — soft 30 over). Вынесено: `circularArrowShape` / `linearArrowShape` (блок-стрелка + narrow fallback), `buildCircularLabels` / `buildLinearLabels`, `circularFeatureArc` (arc/sector math), `bpToLinearX`, `circularTick` (ruler ticks), `originMarkerGeom`, `featureGetsArrow`. TDD: `lib/__tests__/plasmid-mini-map-geometry.test.js` 25 тестов (red→green); reg-guards PlasmidMiniMap*.test.jsx + overview-tab*.test.jsx зелёные.
- **Size budget:** новых hard-нарушителей нет; новый `.js` 10.5 KB под лимитами.
...
- **TD-ASM-PRODUCT-OVERLAP** (✅ ЗАКРЫТО как НЕ-БАГ — V167, 21.06.2026, состязательно проверено): ранее считалось, что `computeAssemblySequence` (`parts.join('')`) удваивает общий регион на стыке. **Adversarial-workflow (3 верификатора, конкретные примеры Gibson/RE/blunt) единогласно: продукт КОРРЕКТЕН для ВСЕХ методов, дельта на стык = 0 байт.** Причина: `segment.sequence` = чистый source-слайс `[start,end)` (+RC/мутации) — homology-плечо живёт ТОЛЬКО в primer-ТЕЙЛЕ (`buildOverlapTail` берёт neighbour-slice в хвост, никогда не сворачивается в хранимый слайс), а RE-оверхенг учтён топ-рез-конвенцией (принадлежит одному соседу). Дедупить нечего — каждый общий base уже хранится ровно раз. **Два инварианта-гардрейла (держат корректность, НАРУШАТЬ НЕЛЬЗЯ):** **INV-1** RE-слайсы остаются топ-рез `[cutA,cutB]` — НЕ расширять (расширишь → удвоишь RE-сайт +оверхенг bp/стык; это ровно то, что защитил V159). **INV-2** primer-тейлы НЕ сворачивать в хранимый слайс (свернёшь → удвоишь overlap-гомологию). Реф — BUGS V159/V167, bio-invariants Правило 2/3. (Документация-гардрейл, не код.)
