...
- **TD-ANNOTATIONTRACK-DECOMPOSE-V2** (OPEN, hard violation): `components/SequenceView/tracks/AnnotationTrack.jsx` ≈ **41.6 KB** (hard 40 KB +1.6 KB over). M-X.2-fix K4 deferred Code'ом — tightly-woven render body.
  - **Эффект:** hard violation ⚓ ANCHORS.md 22.04.2026. Любая правка одной фичи (sub-features rendering / SBOL glyphs / drag preview / hover indicators / multi-line chevrons / predicted-vs-confident styling) рискует ломать другую — single jumbo SVG render body. Смежные правки в M-X.3 wrap-tail rendering / M-X.4 visual save flow / M-D Container Window попадут именно сюда.
  - **Fix:** extract `<RegionRect>` (~150 строк JSX — main rect + chevron + drag preview + glyph-paired-with-label + edge handles + hover indicators) + `<SubFeatureOverlays>` (~80 строк — sub-features inset rendering, шading, label inside) + `<OverflowPill>` (~30 строк — `+N more` indicator) + `useAnnotationStacker` хук (~80 строк — stacking math, height computation). Helpers `darkenColor` / `chevronPath` / `ensureColor` / `labelLengthChars` либо оставить локально, либо в `lib/annotation-render.js` если ≥3 потребителя. Цель — `AnnotationTrack.jsx` ≤ 22 KB, `<RegionRect>` ≤ 8 KB, sub-components ≤ 3 KB. **Требует визуальной приёмки** (рисует то же самое — рисковая зона).
  - **Окно:** перед M-X.3 «Wrap-tail rendering» либо M-X.4 «Library Save Flow» (тот sprint, который первым тронет AnnotationTrack render). Если ни один не трогает — отдельный M-X.2.2 polish sprint после первого acceptance цикла где биолог стабильно использует существующий drag/sub-features/SBOL UX.
  - **Фикс.:** 05.05.2026 (выявлено M-X.2-fix K4 deferred).
