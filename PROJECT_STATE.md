...
> **Версия:** **v0.7.4** — M-X.3 Wrap-tail post-acceptance polish rounds 12–18 (06.05.2026): trailing wrap-tail restore с offset через bridge, bridge wrap-half emits extended-domain caret, paint-only inset accent stripe (CSS layout shift fix), common-features dedup (canonical-key collapse, 419→408), Annotator click→teleport в SequenceView, empty-plasmid LinearFeatureBar, ghost feature edit→save через CREATE conversion в FeatureEditorFlow.
> **Тесты:** Vitest ~1455/1455 passing (без net change vs v0.7.3). pytest 112/112. Build clean, PWA precache 875.76 KiB.
> **Архитектура:** `docs/ARCHITECTURE_v2.md` v1.2 (~117 KB) · 52 ⚓ fundamental decisions в `ANCHORS.md` · sprint-level DEC в `DECISIONS.md` — v0.7.4 sprint block: DEC-WRAPTAIL-04 (trailing offset через bridgeWrapped), DEC-WRAPTAIL-05 (bridge wrap-half extended-domain caret), DEC-LAYOUT-PAINT-ONLY-01 (paint-only decorations через box-shadow inset), DEC-COMMON-FEATURES-DEDUP-01 (canonical-key collapse h=2 hash), DEC-ANN-LOCATE-01 (onLocate prop chain), DEC-LINEAR-BAR-EMPTY-01 (bar always render), DEC-FEATURE-GHOST-EDIT-01 (CREATE conversion для ghost). **⚓ кандидаты в ANCHORS.md** (промоция при reuse в M-X.5): DEC-EDIT-PARITY-01 (подтверждён round-18 — готов к промоции как ⚓ DEC-LIB-14), DEC-IMPORTER-TARGETS-01 (готов к промоции как ⚓ DEC-LIB-15), DEC-LIB-11 (Library entry annotations frozen, mutable через explicit save flow only — supersede candidate в M-X.5 K7), DEC-PARSER-UNIFY-01..03 (от v0.7.1).
> **Журнал версий:** `RELEASES.md` (v0.7.4 + v0.7.3 + v0.7.2 + v0.7.1 + v0.7.0) + `docs/archive/SESSIONS_2026_Q2.md` (исторические сессии до v0.6) + `docs/archive/PROJECT_STATE_v0.6.3_pre_split.md` (полный pre-split snapshot).
> **Дизайн-система:** `docs/DESIGN_SYSTEM.md` §2.1 — feature palette A+v2 + shade-by-name + canonical-key (от v0.6.4). v0.7.4 добавляет паттерн **paint-only decorations через box-shadow inset** для accent stripes над absolute-coord overlays — DEC-LAYOUT-PAINT-ONLY-01. Animation token convention — single `.importer-tab-pane` / `.annotator-drill-in-anim` / `.save-flash-bounce` shared across surfaces, все gated `prefers-reduced-motion`.
> **Открытые TD:** см. `TECH_DEBT.md`. v0.7.4 не закрыл новых TD (rounds 12–18 — pure polish). От v0.7.3: TD-WRAP-BRIDGE-WRAP-AWARE-TRACKS (4 tracks не render features в bridge wrap-half), TD-CIRCULAR-SELECTION (round-8 partial). От v0.7.2: TD-ANNOTATIONTRACK-DECOMPOSE-V2 (41.6 KB hard violation, переносится в M-X.6), TD-SINGLEINSPECTOR-SELECTIONSTATE-EXTRACT, TD-LIBRARY-WRITE-API (закроется M-X.5 K7). От v0.7.1: TD-SEQUENCEVIEW-SHIFT-SELECTION, TD-SEQUENCEVIEW-FOCUS-RING, TD-LINEAR-BAR-PREDICTIONS. От v0.7.0: TD-DRAG-DROP-LIBRARY-CARDS, TD-PER-CDS-SIGNALIP.
> **Открытые баги:** см. `BUGS.md` — OPEN секция пуста.
> **Текущая задача:** см. `CURRENT_TASK.md` — **v0.7.4 финализирован 06.05.2026, M-X.5 Library as Primary Workspace kickoff approved**. План реализации: `~/.claude/plans/delightful-hugging-backus.md` — 2 этапа: Refactor v0.7.5 (K1-3+K9) → Features v0.8.0 (K4-8+K10-11+K12). Спека: `docs/SPRINT_M-X.5_LIBRARY_AS_WORKSPACE.md`. Q1-Q5 закрыты (hybrid catalog, heuristic origin migration, per-mount confirm, name(v2) collision, hard fail на soft-deleted parent).
...
**v0.7.4 закрыт полностью** (M-X.3 wrap-tail rounds 12–18: trailing wrap-tail restored, bridge wrap-half extended-domain caret, paint-only inset accent stripe — CSS layout shift fix, common-features dedup 419→408, Annotator click→teleport, empty-plasmid LinearFeatureBar, ghost feature edit/save через CREATE conversion).
