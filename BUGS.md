...
**V92 — Панели, открываемые кнопкой «Палитра», невозможно закрыть** (OPEN, зафиксирован 21.05.2026, найден на визуальной приёмке M-CANVAS-WORKFLOW-UX). Функциональный дефект.
- **Симптом:** при нажатии «Палитра» в редакторе сборки появляются панель «Схема сборки» и соседние колонки (фильтр контейнеров, инспектор сегмента). Закрыть/свернуть их нельзя — нет close-контрола, повторное нажатие «Палитра» не убирает. Загромождают интерфейс, состояние без выхода.
- **Подход для Code:** «Палитра» = полноценный toggle (повторное нажатие скрывает открытое), либо каждая панель получает close/collapse. Биолог должен мочь вернуть редактор к чистому виду.
...
**V87 — В пикере «Выбор фрагмента» сиквенс за границей выбранного диапазона не затеняется** (OPEN, зафиксирован 21.05.2026, найден на визуальной приёмке M-CANVAS-WORKFLOW-UX).
- **Симптом:** в range picker (выбор фрагмента из плазмиды) сиквенс после маркера-разделителя «конца» фрагмента (жёлтый вертикальный divider с бейджем "1") рендерится тем же контрастом, что и выбранная часть. Должен быть затенён — как нижний wrap-блок, который dimming получает корректно. Непоследовательное затенение. Воспроизводится повсеместно (во всех контейнерах).
- **Подход для Code:** dimmed-стиль out-of-range сиквенса привязать к выбранному [start,end] диапазону, а не к блоку рендера; применить тот же стиль, что уже работает на wrap-блоке.
...
- **Решение (Игорь):** изменение цвета — по клику на цветной квадратик в строке «Источник»; цветовую функцию из «Палитры» убрать.
- **Подход для Code:** color-swatch в строке «Источник» → open color picker. «Палитру» освободить от цветовой легенды. Судьбу «Цвет» в инспекторе «Сегмент» (оставить / свести к swatch) согласовать с Игорем при сборке задания.

**V94 — Инспектор «Сегмент» дублирует нижнюю панель «Источник» — свести в одну** (OPEN, зафиксирован 21.05.2026, найден на визуальной приёмке M-CANVAS-WORKFLOW-UX; поглощает V93).
- **Симптом:** правый инспектор «Сегмент» (Источник / Диапазон / Reverse complement / Цвет / Метка / Применить-Удалить) и нижний список «Источник» — две панели для одного и того же сегмента. Отдельная правая колонка избыточна.
- **Решение (Игорь):** весь раздел «Сегмент» реализовать в нижней панели — редактирование диапазона / RC / цвета / метки прямо в строке «Источник».
- **Подход для Code:** правый инспектор «Сегмент» как отдельную колонку убрать; функции встроить в строку нижнего списка (expandable row или inline-контролы). Цвет — по V93. Освобождённое место справа согласовать с Игорем при сборке задания.

**V95 — Панель «Фильтр по контейнерам проекта» — неясное назначение, громоздкая** (OPEN, зафиксирован 21.05.2026, найден на визуальной приёмке M-CANVAS-WORKFLOW-UX).
- **Симптом:** средняя панель «Фильтр по контейнерам проекта» (поиск + список контейнеров проекта) — назначение не считывается, занимает крупную колонку, выглядит нагромождением. Вероятно тот же контейнер-пикер, что в V85.
- **Решение (Игорь, направление):** либо реализовать через строку поиска вверху, либо сделать компактнее и функциональнее. Финал — выбрать с Игорем.
- **Подход для Code:** назначение панели (источник контейнеров для strip) сделать явным; рассмотреть слияние с верхним поиском / компактный режим. Направление согласовать при сборке задания.
